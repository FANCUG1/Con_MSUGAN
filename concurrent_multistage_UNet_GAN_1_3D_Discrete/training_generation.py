...
def init_G(opt):
    netG = U_Net_3D.GrowingGenerator(opt).to(opt.device)
    netG.apply(U_Net_3D.weights_init)
    return netG


def init_D(opt):
    netD = U_Net_3D.Discriminator(dim=32).to(opt.device)
    netD.apply(U_Net_3D.weights_init)
    return netD
...
def train(opt):
    ...
    for scale_num in range(opt.stop_scale + 1):
        ...
        functions.save_image_as_txt(reals[scale_num], opt, scale_num)
...
def train_single_scale(netD, netG, reals, fixed_noise, noise_amp, opt, depth):
    ...
    if depth == 0:
        ...
        z_opt = functions.generate_noise(size=[opt.nc_im, reals_shapes[depth][2], reals_shapes[depth][3], reals_shapes[depth][4]],
                                         device=opt.device)
