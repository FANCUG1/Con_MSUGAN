...
def upsample(x, size):
    x_up = F.upsample(input=x, size=size, mode='bilinear', align_corners=True)  # mode为bicubic
    return x_up
...
class Discriminator(nn.Module):
    def __init__(self, dim):
        ...
        self.image_to_features = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=dim, kernel_size=(4, 4), stride=(1, 1), padding=(1, 1)),
            nn.LeakyReLU(0.2),
