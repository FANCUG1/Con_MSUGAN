...
def tackle_data(curr_real):
    ...
    curr_real = curr_real.reshape((x, y, z))
    curr_real = curr_real.reshape((x * y * z, 1))
    ...
    curr_real = curr_real.reshape((x, y, z))
    curr_real = curr_real.reshape((1, 1, x, y, z))
...
def read_image(opt):
    ...
    for i in range(3, len(data)):
        x.append(int(data[i]))
    x = np.array(x)
    x = x.reshape((data_size[2], data_size[1], data_size[0], 1))
    x = np2torch(x, opt)
    return x
...
def adjust_scale2image(real_, opt):
    ...
    opt.scale1.append(min(opt.max_size / real_.shape[4], 1))
    ...
    opt.scale_factor.append(math.pow(opt.min_size / real.shape[4], 1 / opt.stop_scale))
    return real
...
def sample_random_noise(depth, reals_shapes, opt):
    ...
    for d in range(depth + 1):
        noise.append(
            generate_noise(size=[opt.nc_im, reals_shapes[d][2], reals_shapes[d][3], reals_shapes[d][4]], device=opt.device).detach())
    return noise
